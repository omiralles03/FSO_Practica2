...
typedef struct
{ /* per una entrada de la taula de posicio */
	int f;
	int c;
} pos;
...
void inicialitza_joc(void)
{
    ...
    for (int i = 0; i < num_opo; i++) {
        ...
        opo[i].d = 1;
        win_escricar(opo[i].f, opo[i].c, i + '1', INVERS); /* escriu la primer posicio oponent */
...
void mou_oponent(int index)
{
	...
	do {
		...
		if (fi2 == 0) /* si no ha col.lisionat amb res */
		{
			...
			win_escricar(opo[index].f, opo[index].c, '1' + index, INVERS); /* dibuixa bloc oponent */
			p_opo[n_opo].f = opo[index].f;					 /* memoritza posicio actual */
			p_opo[n_opo].c = opo[index].c;
			n_opo++;
            escriu_log(index + 1, opo[index].f, opo[index].c, opo[index].d, fi2);
		}
		else
...
int main(int n_args, const char *ll_args[])
{
    ...
    if (n_args == 6) {
        ...
        if (max_ret > 1000) max_ret = 1000;
        if (min_ret < 10) min_ret = 10;
        if (max_ret < min_ret) {
            int aux_ret = max_ret;
            max_ret = min_ret;
            max_ret = aux_ret;
